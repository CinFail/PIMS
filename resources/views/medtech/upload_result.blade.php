...
@section('content')
    ...
    <p class="page-subtitle">
        Test: <strong>{{ $item->test?->test_name }}</strong> —
    ...
    <form action="{{ route('medtech.lab.result.store', $item->request_item_id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="result_value" id="result_value" value="{{ old('result_value', $item->result?->result_value) }}">
        </div>
        <div class="form-group">
            <label for="unit">Unit</label>
            <input type="text" name="unit" id="unit" value="{{ old('unit', $item->result?->unit ?? $item->test?->default_unit) }}">
        </div>
        <div class="form-group">
            <label for="reference_range">Reference Range</label>
            <input type="text" name="reference_range" id="reference_range" value="{{ old('reference_range', $item->result?->reference_range ?? $item->test?->default_reference_range) }}">
        </div>
        <div class="form-group">
            <label for="abnormal_flag">Flag</label>
            <select name="abnormal_flag" id="abnormal_flag">
                @foreach(['Normal','High','Low','Critical'] as $flag)
                    <option value="{{ $flag }}" @selected(old('abnormal_flag', $item->result?->abnormal_flag)==$flag)>{{ $flag }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
